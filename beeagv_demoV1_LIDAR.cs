            ...
            while (startFlag)
            {
                ...
                for (int lidar_angle = 61; lidar_angle < 299; lidar_angle++)       //61 and 299 for convenient
                {
                    ...
                    if (distances[temp_x] * distances[temp_x + 1] > 0) {
                        ...
                        else DataArray[lidar_angle] = ((int)distances[temp_x] + (int)distances[temp_x + 1]) / 2;
                    }
                    else DataArray[lidar_angle] = 0;
                    //DataArray[lidar_angle] = ((int)distances[temp_x - 1] + (int)distances[temp_x] + (int)distances[temp_x + 1]) / 3;
                    if (DataArray[lidar_angle] > 0 && DataArray[lidar_angle] < 20) DataArray[lidar_angle] = 0;        //see small value as noise
