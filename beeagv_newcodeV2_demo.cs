...
namespace lscm.project.followerv2
{
    public class FollowerV2
    {
        ...
        private void FollowerWorkHandler()
        {
            ...
            int safe_distance = 900;        //safe_distance from obstacle to lidar. Note that the lidar is 500mm from car edge 800
            int release_distance = 500;
            int emergency_distance = 850;   //750
            int Ddistance = 0;
        ...
        private void FollowerWorkHandler2()
        {
            ...
            //lidar detect thresh
            double uwb_detect_thresh = 1050;    //threshold for uwb stop (distance)
            ...
            while (_startFlag)
            {
                ...
                if (side_counterL >= DECIDER_NUM || side_counterR >= DECIDER_NUM)       //if obstacles exist on both sides, slow down
                {
                    speed_param = -0.3;
                    uwb_paramL = 0;
                    uwb_paramR = 0;
                ...
                {
                    avoid_paramL = 0.02 * avoid_counterL;
                ...
                if (avoid_counterL >= DECIDER_NUM)
                {
                    avoid_paramR = 0.02 * avoid_counterR;
